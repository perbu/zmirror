...
def zendesk_support(
    ...
    def ticket_comments_table(
        zendesk_client: ZendeskAPIClient,
    ) -> Iterator[TDataItem]:
        """
        Fetches comments for recently updated tickets.

        Uses the incremental ticket export to find which tickets changed,
        then fetches all comments for each. Tracks progress via dlt resource
        state so only tickets updated since last sync are processed.

        Args:
            zendesk_client: The Zendesk API client instance.
    ...
    resource_list = [
        ...
        ticket_comments_table(zendesk_client=zendesk_client),
